...
var residues = []struct {
	exponent                 uint64
	bit_32_to_bit_63_residue uint32
}{
...
func TestMersenneResidues(t *testing.T) {
	to_test := len(residues)
	if testing.Short() {
	...
	for i := 0; i < to_test; i++ {
		...
		m := NewMersenne().AutoInitialise(0, exponent)
		m.Run(400)
		residue := m.Residue()
