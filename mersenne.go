...
func mersenne_initialise() bool {
	...
	s := mod_pow(7, (MOD_P-1)/192/uint64(n)*5)
	...
	// (w+1)*2+log_n >= 63
	if 2*w+uint64(log_n) >= 61 {
		return false
	}

	digit_width0 = uint8(w)
	digit_width_0_max = uint32(1) << w
	digit_width1 = uint8(w) + 1
